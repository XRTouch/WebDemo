...
loadertexture.load('texture/metal/diff.jpg', function(img) {
    centre.map = img;
    centre.needsUpdate = true;
});
loadertexture.load('texture/metal/nor.exr', function(img) {
    centre.normalMap = img;
    centre.needsUpdate = true;
});
loadertexture.load('texture/metal/rough.jpg', function(img) {
    centre.roughnessMap = img;
    centre.needsUpdate = true;
});

droit.propriete = { durete: 2 }; //bois

loadertexture.load('texture/wood/diff.jpg', function(img) {
    droit.map = img;
    droit.needsUpdate = true;
});
loadertexture.load('texture/wood/nor.exr', function(img) {
    droit.normalMap = img;
    droit.needsUpdate = true;
});
loadertexture.load('texture/wood/rough.jpg', function(img) {
    droit.roughnessMap = img;
    droit.needsUpdate = true;
});
...
function render(time) {
    ...
    ciseaux.setRotation(camera.rotation);
    ciseaux.update(dt);
}